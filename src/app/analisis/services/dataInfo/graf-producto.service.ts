...
export class GrafProductoService {
  ...
  private infoCirculoSubject: BehaviorSubject<IClasStats> = new BehaviorSubject<IClasStats>({
    net: 0,
    pos: 0,
    neg: 0,
    total: 0
  ...
  obtenerData(filtro: IComentariosFiltros,filtroTema: number[],filtroSentimiento: number[]) {
    this.localTema = filtroTema;
    if (this.userName !== '') {
      this.cargando = true;
        ...
        this.topicModelingService.getTemasByComentarios(this.userName,listaIDs,4,filtroTema).subscribe((respTopic: any) => {
          this.dataWordCloudSubject.next(respTopic.temas);       

          this.sentimentPredictService.buildStast(this.userName, respTopic.comentariosId, filtroSentimiento).subscribe((respModelIA: any) => {
            this.infoCirculoSubject.next(respModelIA.infoCirculo);
            this.dataComentariosSubject.next(respModelIA.comentarios);
            
            const comnetsIds = respModelIA.comentarios.map( (objeto:any) => objeto.id);
  ...
  private obtenerDataBarFecha(comnetsIds:string[]){
    if (this.userName !== ''){
      this.sentimentPredictService.getComentariosGroupFechaFiltroIds(this.userName,comnetsIds).subscribe( (comGroup:any) =>{
        
        const resulList:IBarInfo[] = this.iterateDataComentariosFecha(comGroup);
